...
| **V12** | **The same six buttons steal the viewpoint selection highlight.** `.btn-viewpoint.active` is amber (`styles.css:543`). The handler clears `active` from every `.btn-viewpoint` and sets it on the clicked one *before* reading `data-viewpoint`, so opening a cabinet door or pressing Front View un-highlights the real current viewpoint and highlights itself. Pre-existing; **not** introduced by the Step 5 guard, which sits after that block by design. Cosmetic. Proper fix is to stop sharing the class — folded into Step 14. | **new — confirmed structurally** |
